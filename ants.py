...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            place.ant = self
         # if self can contain the ant at place, we contain it and then point place.ant at the container ant
        elif self.can_contain(place.ant):
            self.contain_ant(place.ant)
            place.ant = self
        # if the place already has a container ant we put the new ant into it
        elif place.ant.can_contain(self):
            place.ant.contain_ant(self)
            # don't need place.ant = self b/c place is already the container ant
        else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        place = self.place
        counter = 0
        while not (place is beehive):
            bees = place.bees
            if bees and self.min_range <= counter <= self.max_range:
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if not self.imposter:
            # preforms the action of throw ant
            ThrowerAnt.action(self, gamestate)
            # moves place back one to check if there is an ant there
            place = self.place.exit
            # while the places exit is not pointed at the ants base, loop will run
            while place.exit != None:
                # check if there is an ant
                if place.ant != None:
                    # check if the ant is a container ant
                    if isinstance(place.ant, ContainerAnt):
                        # check if it contains an ant and if the contained ant has not already been doubled
                        if place.ant.contained_ant != None and place.ant.contained_ant not in QueenAnt.buffed_lst:
                            # doubles the damage of the contained ant
                            place.ant.contained_ant.damage *= 2
                            # adds the contained ant to the list of ants that have been doubled
                            QueenAnt.buffed_lst.append(place.ant.contained_ant)

                    # check if the ant at place is in not in the list of buffed ants
                    if place.ant not in QueenAnt.buffed_lst:
                        # doubles the damage of the ant at place
                        place.ant.damage *= 2
                        # adds the ant at place to the list of buffed ants
                        QueenAnt.buffed_lst.append(place.ant)
                # move to next place towards ant base
                place = place.exit
        else:
            # if is the imposter, it gets removed instantly
